...
public class ChessBoardUI{
	...
	//02_CONSTRUCTOR
	//*************************************************
	public ChessBoardUI() throws FileNotFoundException{
			...
			if(row%2==0){
				for(int col=1; col<=8; col++){
					if(col%2==0){
						...
						innerBoard.add(blackBlocks[pointer2-1], row-1, col-1);
						boardCells.put(blackBlocks[pointer2-1], new Integer[]{row-1, col-1});
						pointer2++;
					}
				}
			}else{
	...
	private void setActionListeners(){
		...
		for(Rectangle shape:blackBlocks){
			setCellEvents(shape);
		}
		//Board Pieces
